...
def test_get_account_transactions(client, auth_headers):
    ...
    # Get transactions for ACC001 (should have 2: one as sender, one as receiver)
    resp = client.get("/accounts/ACC001/transactions", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["count"] == 2
    assert any(tx["from"] == "ACC001" for tx in data["transactions"])
    assert any(tx["to"] == "ACC001" for tx in data["transactions"])

    # Get transactions for ACC002 (should have 2: one as receiver, one as sender)
    resp = client.get("/accounts/ACC002/transactions", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["count"] == 2

    # Get transactions for ACC003 (should have 2: one as receiver, one as sender)
