...
def test_get_account_transactions(client, auth_headers):
    ...
    # ACC001 should have 2 transactions (sender in first, receiver in third)
    ...
    assert any(tx["from"] == "ACC003" and tx["to"] == "ACC001" for tx in data["transactions"])

    # ACC002 should have 2 transactions (receiver in first, sender in second)
    resp = client.get("/accounts/ACC002/transactions", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["count"] == 2

    # ACC003 should have 2 transactions (receiver in second, sender in third)
