...
def test_get_account_transactions(client, auth_headers):
    ...
    # ACC001 should have 2 transactions (sent one, received one)
    ...
    # ACC002 should have 2 transactions (received one, sent one)
    resp = client.get("/accounts/ACC002/transactions", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["count"] == 2

    # ACC003 should have 2 transactions (received one, sent one)
